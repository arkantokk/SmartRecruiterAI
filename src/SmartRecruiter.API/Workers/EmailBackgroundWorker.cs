...
public class EmailBackgroundWorker : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
                ...
                using (var scope = _scopeFactory.CreateScope())
                {
                    ...
                    foreach (var integration in integrations)
                    {
                        try 
                        {
                            ...
                            {
                                // FIX: Bypass SSL Certificate validation for local development/antivirus issues
                        ...
                        {
                            // Catch errors specific to THIS account so others can still be processed
                            var errorMsg = $"[ERROR] Failed processing account {integration.ConnectedEmail}: {ex.Message}";
                            Console.WriteLine(errorMsg);
                            _logger.LogError(ex, $"Error processing email account {integration.ConnectedEmail}");
                        }
            ...
            {
                // Catch global/critical errors (like DB connection failure)
                var errorMsg = $"[FATAL ERROR] {DateTime.Now:HH:mm:ss}: {ex.Message}";
                Console.WriteLine(errorMsg);
                _logger.LogError(ex, "Background worker critical error occurred.");
            }
