    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                var queueService = scope.ServiceProvider.GetRequiredService<CandidateQueueService>();
                        ...
                        using (var client = new ImapClient())
                        {
                            ...
                            if (uids.Count > 0)
                            {
                                ...
                                var summaries = await inbox.FetchAsync(uids,
                                    MessageSummaryItems.Envelope | MessageSummaryItems.UniqueId, stoppingToken);
                                ...
                                foreach (var summary in summaries)
                                {
                                    ...
                                    if (!hasPdf && fullDossier.Length <= 100) continue;
                                    ...
                                    Console.WriteLine($"[SUCCESS] Added to queue: {request.Email}");
                                }
                            }
